...
//                    ajax 获取境内代理企业列表
                    $.post("recordAction_getAgentJson",
                        ...
                        function (data) {
                            ...
                            $('#agentTable input').remove();

                            $('#agentTable').append("<input type='hidden' id='entidtoseek' value='" + rowData['id'] + "' />");

                            $.each(data, function (i, n) {
                                var str = "<tr onclick='seeDetail(this)'>";
                                str += "<td class='center' name='agentNo'>" + n['agentNo'] + "</td>";
...
//            TODO : 注意这个 id 是展览会表头的id 不是展商id
            ...
            $('#fm').form('submit', {
                url: 'recordEntAction_saveAgent?headid=${id}',
        ...
        function saveAgent() {
//            获取添加展商的 id
            var entid = $('#entidtoseek').val();

            $.messager.progress();	// 显示进度条
            $('#afm').form('submit', {
                url: 'recordEntAction_saveAgent?entid=' + entid,
...
<%-- ------------------------------------------------------------------- --%>
<div id="agentWindow" class="easyui-dialog" style="width:400px;height:280px;padding:10px 20px"
     closed="true" buttons="#dlg-buttons">

    <form id="afm" method="post" novalidate>

        <div style="margin-top: 5px">
            <input name="agentNo" class="easyui-numberbox" required="true" label="序号:"
                   value="" labelPosition="left" style="width: 240px">
        </div>
        <div style="margin-top: 5px">
            <input name="agentCode" class="easyui-textbox" required="true" label="境内报关代理:"
                   value="" labelPosition="left" style="width: 240px">
        </div>
        <div style="margin-top: 5px">
            <input name="agentName" class="easyui-textbox" required="true" label="报关代理名称:"
                   value="" labelPosition="left" style="width: 240px">
        </div>
    </form>
</div>
<div id="dlg-buttons">
    <!--TODO: 添加增加展商的方法 -->
    <a href="javascript:void(0)" class="easyui-linkbutton c6" iconCls="icon-ok" onclick="saveAgent()"
       style="width:90px">Save</a>
    <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-cancel"
       onclick="javascript:$('#agentWindow').dialog('close');$('#afm').form('clear');" style="width:90px">Cancel</a>
</div>
...
<div style="height: 90px;">
    ...
    <input type="button" value="新增" class="tdBtn" style="margin-left: 10px;"
           onclick="javascript: $('#agentWindow').dialog('open').dialog('setTitle', '添加代理商');"/>
...
<%-- ------------------------------------------------------------------- --%>
<div id="billWindow" class="easyui-dialog" style="width:400px;height:280px;padding:10px 20px"
     closed="true" buttons="#dlg-buttons">

    <form id="bfm" method="post" novalidate>

        <div style="margin-top: 5px">
            <input name="gNo" class="easyui-numberbox" required="true" label="序号:"
                   value="" labelPosition="left" style="width: 240px">
        </div>
        <div style="margin-top: 5px">
            <input name="codeTs" class="easyui-textbox" required="true" label="商品编码:"
                   value="" labelPosition="left" style="width: 240px">
        </div>
        <div style="margin-top: 5px">
            <input name="gName" class="easyui-textbox" required="true" label="商品名称:"
                   value="" labelPosition="left" style="width: 240px">
        </div>
    </form>
</div>
<div id="dlg-buttons">
    <a href="javascript:void(0)" class="easyui-linkbutton c6" iconCls="icon-ok" onclick="saveBill()"
       style="width:90px">Save</a>
    <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-cancel"
       onclick="javascript:$('#billWindow').dialog('close');$('#bfm').form('clear');" style="width:90px">Cancel</a>
</div>
...
<div style="height: 30px">
    ...
    <input type="button" value="新增" class="tdBtn" style="margin-left: 10px;" onclick="javascript: $('#billWindow').dialog('open').dialog('setTitle', '添加备案商品');"/>
    <input type="button" value="删除" class="tdBtn" style="margin-left: 10px;" onclick="reloadBill()"/>
...
<script type="text/javascript">
    function saveBill() {
        $('#bfm').form('submit', {
            url: 'recordBillAction_saveBill?headid=${id}',
            ...
            success: function (data) {
                ...
                if (data.success) {
                    $('#billWindow').dialog('close');
                    $('#bfm').form('clear');
                    $('#billList').datagrid({url: "recordBillAction_getBillJson?headid=${id}"});
                }
            }
        });
    }
    ...
    function reloadBill() {
        var temp = $('#billList').datagrid('getSelected');
        $.post(
            "recordBillAction_delete",
            {id : temp.id},
            function (data) {
                alert(data.message);
                $('#billList').datagrid({url: "recordBillAction_getBillJson?headid=${id}"});
